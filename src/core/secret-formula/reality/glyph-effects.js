...
export const glyphEffects = {
  ...
  dilationpow: {
    ...
    effect: (level, strength) => 1.1 + Math.pow(level, 0.7) * Math.pow(strength, 0.7) / 20,
    formatEffect: x => format(x, 2, 2),
  ...
  realityglyphlevel: {
    ...
    effect: level => Math.floor(Math.sqrt(level * 250)),
    formatEffect: x => formatInt(x),
    combine: GlyphCombiner.add,
  },
  realitygalaxies: {
    ...
    totalDesc: "All Galaxy strength +{value}",
    shortDesc: "Galaxy Strength +{value}",
    effect: level => 1 + Math.pow(level / 25000, 0.5),
